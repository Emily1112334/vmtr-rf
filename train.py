...
import datetime
from models.tensorVM_Ring import TensorVM_Ring
...
def reconstruction(args):
    ...
    #optimizer = geoopt.optim.RiemannianAdam(grad_vars)
    optimizer = torch.optim.Adam(grad_vars, betas=(0.9,0.99))

    warmup_steps = -1

    if args.model_name != "TensorTT" and args.model_name != "TensorVM_Ring":
        ...
        base_resolutions = [160, 220, 280, 340, 400]
        N_voxel_list = [r ** 3 for r in base_resolutions]
    ...
    L2_reg_weight = args.L2_weight_inital
    ...
    pbar = tqdm(range(args.n_iters), miniters=args.progress_refresh_rate, file=sys.stdout)#progress_refresh_rate=10
    for iteration in pbar:
        ...
        loss = torch.mean((rgb_map - rgb_train) ** 2)#+lambda*(L*app_line)+lambda*(L*density_line)
        ...
        if L1_reg_weight > 0:
            ...
            #loss_reg_L2 = model.density_L2()
            total_loss += L1_reg_weight*loss_reg_L1
            #total_loss += L1_reg_weight*loss_reg_L2
            summary_writer.add_scalar('train/reg_l1', loss_reg_L1.detach().item(), global_step=iteration)
        if L2_reg_weight > 0:
            loss_reg_L2 = model.density_L2()
            #loss_reg_L2 = model.density_L2()
            total_loss += L2_reg_weight*loss_reg_L2
            #total_loss += L1_reg_weight*loss_reg_L2
            summary_writer.add_scalar('train/reg_l1', loss_reg_L2.detach().item(), global_step=iteration)
        ...
        if iteration % args.vis_every == args.vis_every - 1 and args.N_vis!=0 or iteration-1 in upsample_list or iteration in upsample_list  and args.plot_after_upsampling:
        #if iteration % args.vis_every == args.vis_every - 1 and args.N_vis!=0 or iteration in upsample_list and args.plot_after_upsampling:
        ...
        if iteration-1 in upsample_list or iteration in upsample_list and args.plot_after_upsampling:
        #if iteration in upsample_list and args.plot_after_upsampling:    
        ...
        if iteration in update_AlphaMask_list:
            ...
            if iteration == update_AlphaMask_list[0]:
                ...
                if args.dataset_name == 'blender':
                    model.updateaabb(new_aabb)
                L1_reg_weight = args.L1_weight_rest
                print("continuing L1_reg_weight", L1_reg_weight)
            elif not args.ndc_ray: #  and iteration == update_AlphaMask_list[1]:
                # filter rays outside the bbox
                allrays,allrgbs = model.filtering_rays(allrays,allrgbs, wandb= None)
        ...
        if iteration in upsample_ranks_iterations:
            ...
            model.upsample_mera_volume_ranks()
        ...
        if iteration in upsample_list:
            print("=========> upsampling ...")
            model.eval()
            if (args.model_name == "TensorVMSplit" or args.model_name == "TensorCP") and  args.should_shrink:
                n_voxels = N_voxel_list.pop(0)
                reso_cur = N_to_reso(n_voxels, model.aabb, model_name = args.model_name)
            elif args.model_name != "TensorTT"  :#and args.model_name != "TensorVM_Ring"
                n_voxels = N_voxel_list.pop(0)
                # take the third root of of the number of voxels
                reso = int(n_voxels ** (1/3))+1
            ...
            optimizer = torch.optim.Adam(grad_vars, betas=(0.9, 0.99))
            #optimizer = torch.optim.Adam(grad_vars, weight_decay=3)
            """
            compression_info = model.get_compression_values()
            print("compression info", compression_info)
            tb_add_scalars(summary_writer, 'stats',compression_info, global_step=0)"""
