...
import time
import math
#from .quimb.tn_utils import sample_intcoord_tensor3d,sample_generic_3d
...
class ChannelAttention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        w = self.fc(x)
        return x * w
class ViewFeatureCrossAttn(torch.nn.Module):
        def __init__(self, feat_dim, view_dim, hidden=256):
            super().__init__()
            self.net = torch.nn.Sequential(
                torch.nn.Linear(view_dim, hidden),
                torch.nn.ReLU(inplace=True),
                torch.nn.Linear(hidden, feat_dim),
                torch.nn.Sigmoid()
            )
        ...
        def forward(self, features, viewdirs):
            ...
            w = self.net(viewdirs)
            return features * w
class ViewFeaturePEAttn(torch.nn.Module):
    def __init__(self, feat_pe_dim, view_pe_dim, hidden=256):
        super().__init__()
        self.net = torch.nn.Sequential(
            torch.nn.Linear(view_pe_dim, hidden),
            torch.nn.ReLU(inplace=True),
            torch.nn.Linear(hidden, hidden),
            torch.nn.ReLU(inplace=True),
            torch.nn.Linear(hidden, feat_pe_dim),
            torch.nn.Sigmoid()
        )

    def forward(self, feat_pe, view_pe):
        w = self.net(view_pe)
        return feat_pe * w
...
class TensorBase(torch.nn.Module):
    def __init__(self, aabb, gridSize, device, density_n_comp = 8, appearance_n_comp = 24, app_dim = 25,
        ...
        self.app_n_comp = appearance_n_comp#cp时会用到的组件数
        self.app_dim = app_dim
        self.aabb = aabb#指定表示体积或空间范围的边界，通常用于加速渲染计算或采样。
        self.alphaMask = alphaMask
        self.device=device
        self.name = name
        self.app_plane_rank=300
        self.density_plane_rank=300
        self.init_scale = init_scale

        self.fused = fused
        print("Using Fused Sigma and Appearance Features::", fused)

        self.density_shift = density_shift#密度值偏置
        self.alphaMask_thres = alphaMask_thres
        self.distance_scale = distance_scale
        self.rayMarch_weight_thres = rayMarch_weight_thres#光线行进过程中的权重阈值
        self.fea2denseAct = fea2denseAct

        self.near_far = near_far#视锥的近远裁剪平面
        self.step_ratio = step_ratio#采样步长
        ...
        #self.vecMode = [0,1,2] 
        self.rankapp = [14,14,14,14]
        self.rankdensity = [14,14,14,14]
    ...
    def update_stepSize(self, gridSize):
        ...
        self.stepSize=torch.mean(self.units)*self.step_ratio#0.5
    ...
    def feature2density(self, density_features):
        if self.fea2denseAct == "softplus":
            return F.softplus(density_features+self.density_shift)#-10
    ...
    def compute_alpha(self, xyz_locs, length=1):
        ...
        if alpha_mask.any():
            ...
            if self.name == "TensorTT":#or self.name == "TensorVM_Ring"
        ...
        alpha = 1 - torch.exp(-sigma*length).view(xyz_locs.shape[:-1])#原来的
        return alpha
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1):
        ...
        if ndc_ray:
            ...
            dists = torch.cat((z_vals[:, 1:] - z_vals[:, :-1], torch.zeros_like(z_vals[:, :1])), dim=-1)
        viewdirs = viewdirs.view(-1, 1, 3).expand(xyz_sampled.shape)#xyz_sampled[4096,55,3]
        
        if self.alphaMask is not None:#not None
        ...
        if ray_valid.any():
            xyz_sampled = self.normalize_coord(xyz_sampled)
            points_to_sample = xyz_sampled[ray_valid]#[63570,3]
            if self.name == "TensorTT" :#or self.name == "TensorVM_Ring"
            ...
            if self.fused:
                ...
                rgb_valid = None
                xyzmin=points_to_sample.amin(0)
                xyzmax=points_to_sample.amax(0)
        ...
        alphatest_mask = alpha>self.rayMarch_weight_thres
        a=torch.sum(alphatest_mask,dim=1)
        b=torch.sum(a,dim=0)
        app_mask = weight > self.rayMarch_weight_thres
        test=torch.sum(app_mask,dim=1)
        test2=torch.sum(test,dim=0)
        if app_mask.any():
